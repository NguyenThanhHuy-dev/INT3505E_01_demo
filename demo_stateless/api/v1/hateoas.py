...
def generate_book_links(book_id=None, cursor=None, next_cursor=None, offset=None, limit=None, page=None, per_page=None):
    links = {
        "list_page": url_for(f"{BASE}_books_bp.list_books", _external=True),
        "list_offset": url_for(f"{BASE}_books_bp.list_books_offset", _external=True),
        "list_cursor": url_for(f"{BASE}_books_bp.list_books_cursor", _external=True),
        "create": url_for(f"{BASE}_books_bp.create_book", _external=True)
    }
    ...
    if offset is not None:
        links["self"] = url_for(f"{BASE}_books_bp.list_books_offset", offset=offset, limit=limit, _external=True)
        if limit is not None:
            links["next"] = url_for(f"{BASE}_books_bp.list_books_offset", offset=offset + limit, limit=limit, _external=True)
            if offset - limit >= 0:
                links["prev"] = url_for(f"{BASE}_books_bp.list_books_offset", offset=offset - limit, limit=limit, _external=True)
    if page is not None:
        links["self"] = url_for(f"{BASE}_books_bp.list_books", page=page, per_page=per_page, _external=True)
        if per_page and page is not None:
            links["next"] = url_for(f"{BASE}_books_bp.list_books", page=page + 1, per_page=per_page, _external=True)
            if page > 1:
                links["prev"] = url_for(f"{BASE}_books_bp.list_books", page=page - 1, per_page=per_page, _external=True)
...
def generate_user_links(user_id=None):
    links = {
        "list": url_for(f"{BASE}_users_bp.list_users", _external=True),
        "create": url_for(f"{BASE}_users_bp.create_user", _external=True)
    }
    if user_id:
        links.update({
...
def generate_loan_links(loan_id=None, user_id=None, book_id=None):
    links = {
        "list": url_for(f"{BASE}_loans_bp.list_loans", _external=True),
        "create": url_for(f"{BASE}_loans_bp.create_loan", _external=True)
    }
    if loan_id:
        links.update({
